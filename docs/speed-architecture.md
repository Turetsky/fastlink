...
- **fast_locate** — DOM-preferring race. {target} fires DOM (snapshot+match, 3s timeout, .catch→null) AND vision (pointOnce) CONCURRENTLY; pickLocateWinner() prefers the more-precise DOM tier: DOM hit wins outright, DOM miss accepts vision immediately, and a vision hit while DOM is still pending is held through a 1.2s grace window (then accepted). Hard 8s ceiling so a wedged vision call can't hang. Hung GCP DOM can't stall vision. Returns {via:'dom'|'vision'|null, xCss, yCss, found}.
